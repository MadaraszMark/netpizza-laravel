    ...
    <div class="container">
      <div class="row slider-text justify-content-center align-items-center">
        <div class="col-md-7 text-center ftco-animate">
          <h1 class="mb-3 mt-5 bread">Pizza szerkesztése</h1>
          <p class="breadcrumbs">
  ...
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-md-8 bg-white p-5 rounded shadow-lg">
        <h3 class="text-center mb-4">✏️ Pizza módosítása</h3>
        ...
        <form action="{{ route('pizzas.update', $pizza->nev) }}" method="POST" class="contact-form">
          @csrf
          @method('PUT')

          <div class="form-group mb-3">
            <label for="nev" class="font-weight-bold">Pizza neve</label>
            <input type="text" name="nev" class="form-control" value="{{ old('nev', $pizza->nev) }}" required>
          </div>
          ...
          <div class="form-group mb-3">
            ...
            <select name="kategorianev" class="form-control" required>
              ...
              <option value="Klasszikus" {{ $pizza->kategorianev === 'Klasszikus' ? 'selected' : '' }}>Klasszikus</option>
              <option value="Prémium" {{ $pizza->kategorianev === 'Prémium' ? 'selected' : '' }}>Prémium</option>
              <option value="Extra" {{ $pizza->kategorianev === 'Extra' ? 'selected' : '' }}>Extra</option>
            </select>
          </div>
          ...
          <div class="form-group mb-3">
            ...
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="vegetarianus" id="veg1" value="1" {{ $pizza->vegetarianus ? 'checked' : '' }}>
              <label class="form-check-label" for="veg1">Igen</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="vegetarianus" id="veg0" value="0" {{ !$pizza->vegetarianus ? 'checked' : '' }}>
              <label class="form-check-label" for="veg0">Nem</label>
            </div>
          </div>

          <div class="text-center mt-4">
            <button type="submit" class="btn btn-success px-5 py-2">💾 Mentés</button>
